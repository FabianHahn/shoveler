...
typedef bool(ShovelerMaterialRenderFunction)(
...
typedef int(ShovelerMaterialAttachUniformsFunction)(
    ShovelerMaterial* material, ShovelerShader* shader, void* userData);
...
ShovelerMaterial* shovelerMaterialCreate(
    ShovelerShaderCache* shaderCache, bool screenspace, GLuint program);
/** Creates a material from a program without owning it. */
ShovelerMaterial* shovelerMaterialCreateUnmanaged(
    ShovelerShaderCache* shaderCache, bool screenspace, GLuint program);
void shovelerMaterialFree(ShovelerMaterial* material);

static inline bool shovelerMaterialRender(
    ShovelerMaterial* material,
    ShovelerScene* scene,
    ShovelerCamera* camera,
    ShovelerLight* light,
...
static inline int shovelerMaterialAttachUniforms(
    ShovelerMaterial* material, ShovelerShader* shader, void* userData) {
