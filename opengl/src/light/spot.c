...
static int renderSpotLight(
    void* spotlightPointer,
    ShovelerScene* scene,
    ShovelerCamera* camera,
...
ShovelerLightSpotShared* shovelerLightSpotSharedCreate(
  ...
  shared->depthFilter = shovelerFilterDepthTextureGaussianCreate(
      shaderCache, width, height, samples, exponentialFactor);
...
ShovelerLight* shovelerLightSpotCreateWithShared(
    ShovelerCamera* camera, ShovelerLightSpotShared* shared, bool managedShared) {
  ...
  shovelerUniformMapInsert(
      spotlight->light.uniforms, "isExponentialLiftedShadowMap", shovelerUniformCreateInt(1));
  shovelerUniformMapInsert(
      spotlight->light.uniforms,
      "lightAmbientFactor",
      shovelerUniformCreateFloat(spotlight->shared->ambientFactor));
  shovelerUniformMapInsert(
      spotlight->light.uniforms,
      "lightExponentialShadowFactor",
      shovelerUniformCreateFloat(spotlight->shared->exponentialFactor));
  shovelerUniformMapInsert(
      spotlight->light.uniforms,
      "lightColor",
      shovelerUniformCreateVector3Pointer(&spotlight->shared->color));
  shovelerUniformMapInsert(
      spotlight->light.uniforms,
      "lightPosition",
      shovelerUniformCreateVector3Pointer(&spotlight->camera->position));
  shovelerUniformMapInsert(
      spotlight->light.uniforms,
      "lightView",
      shovelerUniformCreateMatrixPointer(&spotlight->camera->view));
  shovelerUniformMapInsert(
      spotlight->light.uniforms,
      "lightProjection",
      shovelerUniformCreateMatrixPointer(&spotlight->camera->projection));
  shovelerUniformMapInsert(
      spotlight->light.uniforms,
      "shadowMap",
      shovelerUniformCreateTexture(
          spotlight->shared->depthFilter->outputTexture, spotlight->shared->shadowMapSampler));
...
static int renderSpotLight(
  ...
  rendered += shovelerSceneRenderPass(
      scene, spotlight->camera, NULL, spotlight->shared->depthRenderPassOptions, renderState);

  // filter depth map
  rendered += shovelerFilterRender(
      spotlight->shared->depthFilter,
      spotlight->shared->depthFramebuffer->depthTarget,
      renderState);

  // render additive light to scene
  shovelerFramebufferUse(framebuffer);
  rendered +=
      shovelerSceneRenderPass(scene, camera, &spotlight->light, renderPassOptions, renderState);
