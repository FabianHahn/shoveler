...
static void tiltController(
    ShovelerController* controller,
    ShovelerVector3 direction,
    ShovelerVector3 upwards,
    void* userData);
static void moveController(
    ShovelerController* controller, ShovelerVector3 position, void* userData);
static void aspectRatioChangeController(
    ShovelerController* controller, float aspectRatio, void* userData);
...
ShovelerCamera* shovelerCameraPerspectiveCreate(
  ...
  shovelerProjectionPerspectiveComputeTransformation(
      &perspectiveCamera->projection, &perspectiveCamera->camera.projection);
...
void shovelerCameraPerspectiveAttachController(
  ...
  perspectiveCamera->controllerTiltCallback =
      shovelerControllerAddTiltCallback(controller, tiltController, perspectiveCamera);
  perspectiveCamera->controllerMoveCallback =
      shovelerControllerAddMoveCallback(controller, moveController, perspectiveCamera);
  perspectiveCamera->controllerAspectRatioChangeCallback =
      shovelerControllerAddAspectRatioChangeCallback(
          controller, aspectRatioChangeController, perspectiveCamera);
}
...
void shovelerCameraPerspectiveDetachController(ShovelerCamera* camera) {
  ...
  shovelerControllerRemoveTiltCallback(
      perspectiveCamera->controller, perspectiveCamera->controllerTiltCallback);
  shovelerControllerRemoveMoveCallback(
      perspectiveCamera->controller, perspectiveCamera->controllerMoveCallback);
  shovelerControllerRemoveAspectRatioChangeCallback(
      perspectiveCamera->controller, perspectiveCamera->controllerAspectRatioChangeCallback);
...
static void updateView(void* perspectiveCameraPointer) {
  ...
  shovelerProjectionPerspectiveComputeFrustum(
      &perspectiveCamera->projection, &frame, &perspectiveCamera->camera.frustum);
}
...
static void moveController(
  ...
  if (perspectiveCamera->logPositionChanges &&
      memcmp(&position, &perspectiveCamera->camera.position, sizeof(ShovelerVector3)) != 0) {
    shovelerLogInfo(
        "Camera at (%.2f, %.2f, %.2f).",
        perspectiveCamera->camera.position.values[0],
        perspectiveCamera->camera.position.values[1],
        perspectiveCamera->camera.position.values[2]);
  }
