...
guint shovelerShaderKeyHash(gconstpointer shaderKeyPointer) {
  ...
  return shovelerHashCombine(
      sceneHash,
      shovelerHashCombine(
          cameraHash,
          shovelerHashCombine(
              lightHash,
              shovelerHashCombine(modelHash, shovelerHashCombine(materialHash, userDataHash)))));
}

gboolean shovelerShaderKeyEqual(
    gconstpointer firstShaderKeyPointer, gconstpointer secondShaderKeyPointer) {
...
bool shovelerShaderAttachUniform(
    ShovelerShader* shader, const char* name, ShovelerUniform* uniform) {
  GLint location = glGetUniformLocation(shader->material->program, name);
  if (!shovelerOpenGLCheckSuccess()) {
    return false;
  } else if (location < 0) {
    shovelerLogTrace(
        "Material %p with shader program %d does not have a uniform attachment point for '%s', "
        "skipping.",
        shader->material,
    ...
    shovelerLogTrace(
        "Material %p with shader program %d already contains an attachment for '%s', skipping.",
        shader->material,
        shader->material->program,
        name);
    return false;
  ...
  shovelerLogTrace(
      "Attached uniform '%s' to material %p with shader program %d.",
      name,
      shader->material,
      shader->material->program);
  return true;
