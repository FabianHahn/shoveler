...
static int attachUniforms(
    ShovelerMaterial* variationMaterial, ShovelerShader* shader, void* userData);
static void freeVariation(ShovelerMaterial* variationMaterial);

ShovelerMaterial* shovelerMaterialVariationCreate(
    ShovelerShaderCache* shaderCache, ShovelerMaterial* delegate) {
  Variation* variation = malloc(sizeof(Variation));
  variation->material =
      shovelerMaterialCreateUnmanaged(shaderCache, delegate->screenspace, delegate->program);
