...
static const char* fragmentShaderSource =
    ...
    ""
    "	// To avoid singularities around 0 and 1, make sure we don't break continuity of the uv "
    "function\n"
    "	// because of flooring.\n"
...
ShovelerMaterial* shovelerMaterialTilemapCreate(
  ...
  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "spritePosition",
      shovelerUniformCreateVector2Pointer(&materialData->activeSpritePosition));
  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "spriteSize",
      shovelerUniformCreateVector2Pointer(&materialData->activeSpriteSize));

  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tilesWidth",
      shovelerUniformCreateIntPointer(&materialData->activeLayerWidth));
  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tilesHeight",
      shovelerUniformCreateIntPointer(&materialData->activeLayerHeight));

  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tiles",
      shovelerUniformCreateTexturePointer(
          &materialData->activeLayerTexture, &materialData->tilesSampler));

  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tilesetId",
      shovelerUniformCreateIntPointer(&materialData->activeTilesetId));

  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tilesetColumns",
      shovelerUniformCreateIntPointer(&materialData->activeTilesetColumns));
  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tilesetRows",
      shovelerUniformCreateIntPointer(&materialData->activeTilesetRows));
  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tilesetPadding",
      shovelerUniformCreateIntPointer(&materialData->activeTilesetPadding));
  shovelerUniformMapInsert(
      materialData->material->uniforms,
      "tileset",
      shovelerUniformCreateTexturePointer(
          &materialData->activeTilesetTexture, &materialData->activeTilesetSampler));

  return materialData->material;
}

void shovelerMaterialTilemapSetActiveRegion(
...
void shovelerMaterialTilemapSetActiveSprite(
    ShovelerMaterial* material, ShovelerSpriteTilemap* sprite) {
  MaterialData* materialData = material->data;
...
void shovelerMaterialTilemapSetActiveTiles(
    ShovelerMaterial* tilemapMaterial, ShovelerTexture* tiles) {
  MaterialData* materialData = tilemapMaterial->data;
...
void shovelerMaterialTilemapSetActiveTileset(
    ShovelerMaterial* tilemapMaterial, int tilesetId, ShovelerTileset* tileset) {
  MaterialData* materialData = tilemapMaterial->data;
...
static bool render(
  ...
  return shovelerTilemapRender(
      materialData->activeTilemap,
      materialData->activeRegionPosition,
      materialData->activeRegionSize,
      material,
      scene,
      camera,
      light,
