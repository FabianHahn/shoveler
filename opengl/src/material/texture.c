...
static const char* fragmentShaderSourcePhong =
    ...
    "		fragmentColor = vec4(worldUv.xy, worldUv.y, 1.0);\n"
    "	} else {\n"
    "		fragmentColor = vec4(clamp(lightAmbientFactor * color + exponentialShadowFactor * "
    "diffuseFactor * color * lightColor + exponentialShadowFactor * specularFactor * lightColor, "
    "0.0, 1.0), 1.0);\n"
    "	}\n"
    "}\n";

ShovelerMaterial* shovelerMaterialTextureCreate(
    ShovelerShaderCache* shaderCache,
    bool screenspace,
    ShovelerMaterialTextureType type,
    ShovelerTexture* texture,
    bool manageTexture,
    ShovelerSampler* sampler,
    bool manageSampler) {
  ...
  GLuint fragmentShaderObject = shovelerShaderProgramCompileFromString(
      type == SHOVELER_MATERIAL_TEXTURE_TYPE_PHONG ? fragmentShaderSourcePhong
  ...
  shovelerUniformMapInsert(
      material->uniforms,
      "color",
      shovelerUniformCreateVector4Pointer(&materialTextureData->color));
  shovelerUniformMapInsert(
      material->uniforms,
      "depthOnly",
      shovelerUniformCreateInt(type == SHOVELER_MATERIAL_TEXTURE_TYPE_DEPTH));
  shovelerUniformMapInsert(
      material->uniforms,
      "alphaMask",
      shovelerUniformCreateInt(type == SHOVELER_MATERIAL_TEXTURE_TYPE_ALPHA_MASK));
  shovelerUniformMapInsert(
      material->uniforms, "textureImage", shovelerUniformCreateTexture(texture, sampler));
