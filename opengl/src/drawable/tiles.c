...
ShovelerDrawable* shovelerDrawableTilesCreate(unsigned char width, unsigned char height) {
  ...
  glVertexAttribFormat(
      ...
      GL_UNSIGNED_BYTE,
      GL_FALSE,
      offsetof(TilesVertex, position));
  glVertexAttribFormat(
      SHOVELER_SHADER_PROGRAM_ATTRIBUTE_UV,
      2,
      GL_UNSIGNED_BYTE,
      GL_FALSE,
      offsetof(TilesVertex, uv));
  ...
  glBufferData(
      GL_ARRAY_BUFFER, 4 * width * height * sizeof(TilesVertex), tiles->vertices, GL_STATIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, tiles->indexBuffer);
  glBufferData(
      GL_ELEMENT_ARRAY_BUFFER,
      2 * width * height * sizeof(TilesTriangle),
      tiles->triangles,
      GL_STATIC_DRAW);
