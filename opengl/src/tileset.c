...
ShovelerTileset* shovelerTilesetCreate(
  ...
  ShovelerImage* paddedImage = shovelerImageCreate(
      image->width + 2 * padding * columns, image->height + 2 * padding * rows, image->channels);
      ...
      for (int i = -padding; i < (int) tileWidth + padding; i++) {
        ...
        for (int j = -padding; j < (int) tileHeight + padding; j++) {
          ...
          for (unsigned int c = 0; c < image->channels; c++) {
            shovelerImageGet(paddedImage, paddedImageI, paddedImageJ, c) =
                shovelerImageGet(image, imageI, imageJ, c);
          }
        }
      }
    }
