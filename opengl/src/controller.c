...
static void keyHandler(
    ShovelerInput* input, int key, int scancode, int action, int mods, void* controllerPointer);
...
static bool tryMoveDirection(
    ShovelerController* controller,
    ShovelerVector3 right,
    float moveAmountRight,
    float moveAmountUp,
    float moveAmountForward);
static void windowSizeHandler(
    ShovelerInput* input, unsigned int width, unsigned int height, void* controllerPointer);
static void triggerTilt(
    ShovelerController* controller, ShovelerVector3 direction, ShovelerVector3 orientation);
...
ShovelerController* shovelerControllerCreate(
  ...
  ShovelerVector3 right =
      shovelerVector3Normalize(shovelerVector3Cross(controller->frame.direction, controller->up));
  ...
  controller->tiltCallbacks =
      g_hash_table_new_full(g_direct_hash, g_direct_equal, freeTiltCallback, NULL);
  controller->moveCallbacks =
      g_hash_table_new_full(g_direct_hash, g_direct_equal, freeMoveCallback, NULL);
  controller->aspectRatioChangeCallbacks =
      g_hash_table_new_full(g_direct_hash, g_direct_equal, freeAspectRatioChangeCallback, NULL);

  controller->keyCallback = shovelerInputAddKeyCallback(controller->input, keyHandler, controller);
  controller->windowSizeCallback =
      shovelerInputAddWindowSizeCallback(controller->input, windowSizeHandler, controller);
...
ShovelerControllerAspectRatioChangeCallback* shovelerControllerAddAspectRatioChangeCallback(
  ...
  ShovelerControllerAspectRatioChangeCallback* callback =
      malloc(sizeof(ShovelerControllerAspectRatioChangeCallback));
  callback->function = callbackFunction;
  callback->userData = userData;
...
void shovelerControllerSetFrame(
  ...
  bool tilted =
      memcmp(&controller->frame.direction, &frame->direction, sizeof(ShovelerVector3)) != 0 &&
...
static void updatePosition(ShovelerController* controller, float dt) {
  ...
  if (moveAmount.values[0] != 0.0f || moveAmount.values[1] != 0.0f ||
      moveAmount.values[2] != 0.0f) {
...
static void updateTilt(ShovelerController* controller, float dt) {
  ...
  ShovelerVector2 tiltAmount = shovelerVector2(
      controller->lockTiltX ? 0.0f : tiltAmountX, controller->lockTiltY ? 0.0f : tiltAmountY);
...
static void shiftPosition(ShovelerController* controller, ShovelerVector3 moveAmount) {
  ...
  if (tryMoveDirection(
          controller, right, moveAmount.values[0], moveAmount.values[1], moveAmount.values[2])) {
    return;
  }
...
static bool tryMoveDirection(
  ...
  targetPosition = shovelerVector3LinearCombination(
      1.0, controller->frame.position, moveAmountForward, controller->frame.direction);
  targetPosition = shovelerVector3LinearCombination(1.0, targetPosition, moveAmountRight, right);
  targetPosition =
      shovelerVector3LinearCombination(1.0, targetPosition, moveAmountUp, controller->frame.up);

  // check for 2d collisions
  if (controller->boundingBoxSize2 > 0.0f) {
    ShovelerVector2 targetPosition2 =
        shovelerVector2(targetPosition.values[0], targetPosition.values[1]);

    ShovelerBoundingBox2 targetBoundingBox2 = shovelerBoundingBox2(
        shovelerVector2LinearCombination(
            1.0,
            targetPosition2,
            -0.5f * controller->boundingBoxSize2,
    ...
    const ShovelerCollider2* collider =
        shovelerCollidersIntersect2(controller->colliders, &targetBoundingBox2);
    if (collider != NULL) {
      shovelerLogTrace(
          "Bumping into 2d collider %p with bounding box (%.2f, %.2f)-(%.2f, %.2f), aborting "
          "position shift to (%.2f, %.2f).",
          collider,
  ...
  if (controller->boundingBoxSize3 > 0.0f) {
    ShovelerBoundingBox3 targetBoundingBox3 = shovelerBoundingBox3(
        shovelerVector3LinearCombination(
            1.0,
            targetPosition,
            -0.5f * controller->boundingBoxSize3,
    ...
    const ShovelerCollider3* collider =
        shovelerCollidersIntersect3(controller->colliders, &targetBoundingBox3);
    if (collider != NULL) {
      shovelerLogTrace(
          "Bumping into 3d collider %p with bounding box (%.2f, %.2f, %.2f)-(%.2f, %.2f, %.2f), "
          "aborting position shift to (%.2f, %.2f, %.2f).",
          collider,
          collider->boundingBox.min.values[0],
          collider->boundingBox.min.values[1],
...
static void shiftOrientation(ShovelerController* controller, ShovelerVector2 tiltAmount) {
  ...
  controller->frame.direction = shovelerVector3Normalize(
      shovelerMatrixMultiplyVector3(normalRotationX, controller->frame.direction));
  ...
  ShovelerVector3 newDirection = shovelerVector3Normalize(
      shovelerMatrixMultiplyVector3(normalRotationY, controller->frame.direction));
  ShovelerVector3 newFrameUp = shovelerVector3Cross(right, newDirection);

  if (shovelerVector3Dot(controller->up, newFrameUp) >
      0.0f) { // only update if we're not flipping upside down
...
static void triggerTilt(
    ShovelerController* controller, ShovelerVector3 direction, ShovelerVector3 upwards) {
  GHashTableIter iter;
...
static void freeTiltCallback(void* tiltCallbackPointer) { free(tiltCallbackPointer); }

static void freeMoveCallback(void* moveCallbackPointer) { free(moveCallbackPointer); }
