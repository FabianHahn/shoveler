...
static void keyHandler(
    ShovelerInput* input, int key, int scancode, int action, int mods, void* unused);
static gint64 elapsedNs(double dt);
static void printFps(void* gamePointer);

ShovelerGame* shovelerGameCreate(
    ShovelerGameUpdateCallback* update,
    const ShovelerGameWindowSettings* windowSettings,
    const ShovelerGameCameraSettings* cameraSettings,
    const ShovelerGameControllerSettings* controllerSettings) {
  ...
  if (windowSettings->fullscreen) {
    ...
    shovelerLogInfo(
        "Using borderless fullscreen mode on primary monitor '%s'.", glfwGetMonitorName(monitor));
  ...
  shovelerLogInfo(
      "Opened glfw window with name '%s', using OpenGL driver version '%s' by '%s' and GLSL "
      "version '%s'.",
  ...
  game->camera = shovelerCameraPerspectiveCreate(
      game->shaderCache, &cameraSettings->frame, &cameraSettings->projection);
  game->colliders = shovelerCollidersCreate();
  game->controller = shovelerControllerCreate(
      game->window,
      game->input,
      game->colliders,
      &controllerSettings->frame,
      controllerSettings->moveFactor,
      controllerSettings->tiltFactor,
      controllerSettings->boundingBoxSize2,
      controllerSettings->boundingBoxSize3);
  ...
  game->screenspaceCanvasMaterial =
      shovelerMaterialCanvasCreate(game->shaderCache, /* screenspace */ true);
  shovelerMaterialCanvasSetActive(game->screenspaceCanvasMaterial, game->screenspaceCanvas);
  updateScreenspaceCanvasRegion(game);
  game->screenspaceCanvasModel =
      shovelerModelCreate(game->screenspaceCanvasQuad, game->screenspaceCanvasMaterial);
...
void shovelerGameToggleFullscreen(ShovelerGame* game) {
  if (game->fullscreen) {
    ...
    glfwSetWindowMonitor(
        game->window, NULL, 0, 0, game->windowedWidth, game->windowedHeight, GLFW_DONT_CARE);

    shovelerFramebufferFree(game->framebuffer, /* keepTargets */ false);
    game->framebuffer =
        shovelerFramebufferCreate(game->windowedWidth, game->windowedHeight, game->samples, 4, 8);
  } else {
    GLFWmonitor* monitor = glfwGetPrimaryMonitor();
    shovelerLogInfo(
        "Switching game %p window to fullscreen mode on primary monitor '%s'.",
        game,
...
int shovelerGameRenderFrame(ShovelerGame* game) {
  ...
  int rendered =
      shovelerSceneRenderFrame(game->scene, game->camera, game->framebuffer, &game->renderState);
...
static void updateScreenspaceCanvasRegion(ShovelerGame* game) {
  ShovelerVector2 regionPosition =
      shovelerVector2(0.5f * game->framebuffer->width, 0.5f * game->framebuffer->height);
  ...
  shovelerMaterialCanvasSetActiveRegion(
      game->screenspaceCanvasMaterial, regionPosition, regionSize);
}
