...
bool shovelerImagePpmWriteFile(ShovelerImage* image, const char* filename) {
  if (image->channels < 3) {
    shovelerLogError(
        "Failed to write PPM image to '%s': can only write image with at least 3 channels.",
        filename);
    return false;
  }
  ...
  shovelerLogInfo(
      "Successfully wrote PPM image of size (%u, %u) to '%s'.",
      image->width,
      image->height,
      filename);
  return true;
...
static ShovelerImage* readPpm(ShovelerInputStream* inputStream) {
  ...
  if (shovelerInputStreamRead(inputStream, header, PPM_HEADER_BYTES) < PPM_HEADER_BYTES) {
    shovelerLogError(
        "Failed to read PPM image from %s: failed to read PPM header.",
  ...
  if (strncmp((const char*) header, ppmHeader, PPM_HEADER_BYTES) != 0) {
    shovelerLogError(
        "Failed to read PPM image from %s: invalid magic number.",
        shovelerInputStreamGetDescription(inputStream));
    return NULL;
  }
  ...
  while (c == '#') {
    ...
    while (c != '\n') {
      if (c == EOF) {
        shovelerLogError(
            "Failed to read PPM image from %s: unexpected end of file when parsing header comment.",
            shovelerInputStreamGetDescription(inputStream));
        return NULL;
      }
  ...
  if (width < 0 || height < 0 || maxValue <= 0) {
    shovelerLogError(
        "Failed to read PPM image from %s: invalid header defining image of size (%d, %d) and max "
        "value of %d.",
        shovelerInputStreamGetDescription(inputStream),
        width,
        height,
  ...
  for (unsigned int y = 0; y < height; y++) {
    for (unsigned int x = 0; x < width; x++) {
      ...
      if (r < 0 || r > maxValue || g < 0 || g > maxValue || b < 0 || b > maxValue) {
        shovelerLogError(
            "Failed to read PPM image from %s: invalid pixel (%d, %d) value of (%d, %d, %d).",
            shovelerInputStreamGetDescription(inputStream),
            x,
            y,
            r,
  ...
  shovelerLogInfo(
      "Successfully read PPM image of size (%d, %d) from %s.",
      width,
      height,
      shovelerInputStreamGetDescription(inputStream));
  return image;
