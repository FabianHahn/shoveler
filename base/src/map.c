  ...
  for (int chunkX = 0; chunkX < numChunkColumns; chunkX++) {
    ...
    for (int chunkY = 0; chunkY < numChunkRows; chunkY++) {
      ...
      if (x == -chunkSize && y == -chunkSize) {
        ...
        createChunk(
            x, y, map->dimensions.halfMapHeight, map->dimensions.halfMapWidth, chunkSize, chunk);
      }
    }
  }
...
ShovelerMapTileData shovelerMapLookupTile(
  ...
  return shovelerMapChunkLookupTile(
      chunk, map->dimensions.chunkSize, tile.tileX, tile.tileY, background);
}
...
void shovelerMapWriteTile(
  ...
  ShovelerMapChunk* chunk = shovelerMapGetChunk(map, tile.chunkX, tile.chunkY);
  shovelerMapChunkWriteTile(
      chunk, map->dimensions.chunkSize, tile.tileX, tile.tileY, background, tileData);
}
...
static void createStartingAreaBottomLeft(int chunkSize, ShovelerMapChunk* outputChunk) {
  ...
  for (int tileY = 0; tileY < chunkSize; tileY++) {
    for (int tileX = 0; tileX < chunkSize; tileX++) {
      ...
      shovelerMapChunkWriteTile(
          outputChunk, chunkSize, tileX, tileY, /* background */ true, background);
      shovelerMapChunkWriteTile(
          outputChunk, chunkSize, tileX, tileY, /* background */ false, foreground);
    }
  }
}
...
static void createChunk(
    ...
    int chunkSize,
    ShovelerMapChunk* outputChunk) {
  outputChunk->position =
      shovelerVector2((float) x + (float) chunkSize / 2.0f, (float) y + (float) chunkSize / 2.0f);
  ...
  for (int tileY = 0; tileY < chunkSize; tileY++) {
    ...
    for (int tileX = 0; tileX < chunkSize; tileX++) {
      ...
      do {
        if (tileY > 0) {
          ShovelerMapTileData previousRowBackground = shovelerMapChunkLookupTile(
              outputChunk, chunkSize, tileX, tileY - 1, /* background */ true);
        ...
        if (tileX > 0) {
          ShovelerMapTileData previousTileBackground = shovelerMapChunkLookupTile(
              outputChunk, chunkSize, tileX - 1, tileY, /* background */ true);
        ...
        if (tileY > 0 && tileX < chunkSize - 1) {
          ShovelerMapTileData previousRowNextTileBackground = shovelerMapChunkLookupTile(
              outputChunk, chunkSize, tileX + 1, tileY - 1, /* background */ true);
          // reject positions that already have a tree starting on the bottom right neighbor tile
          if (previousRowNextTileBackground.tilesetColumn == 3 &&
              previousRowNextTileBackground.tilesetRow == 0) {
            break;
          }
        }
