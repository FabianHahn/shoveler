...
TEST(compression, compressAndDecompress) {
  ...
  for (const auto& testCase : testCases) {
    ...
    bool compressed = shovelerCompressionCompress(
        testCaseFormat,
        (const unsigned char*) testInput,
        testInputSize,
        &testOutput,
        &testOutputSize);
    ...
    bool decompressed = shovelerCompressionDecompress(
        testCaseFormat,
        testOutput,
        testOutputSize,
        &testReconstructedInput,
        &testReconstructedInputSize);
    ASSERT_TRUE(decompressed) << testCaseName << " decompression should succeed";
    ASSERT_EQ(testReconstructedInputSize, testInputSize)
        << testCaseName << " reconstructed input size should match input size";
...
TEST(compression, decompressInvalid) {
  ...
  for (const auto& testCase : testCases) {
    ...
    unsigned char* testOutput;
    size_t testOutputSize;
    bool compressed = shovelerCompressionDecompress(
        testCaseFormat,
        (const unsigned char*) testInput,
        testInputSize,
        &testOutput,
