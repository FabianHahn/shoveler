...
TEST(PositionQuantizer, QuantizeDequantize)
{
	...
	ShovelerPositionQuantizer quantizer =
		shovelerPositionQuantizer(worldMin, worldMax);

	uint64_t quantizedX;
	uint64_t quantizedY;

	// minimum
	shovelerPositionQuantizerFromWorld(
		&quantizer, worldMin, &quantizedX, &quantizedY);
	ASSERT_EQ(quantizedX, 0);
	ASSERT_EQ(quantizedY, 0);

	// maximum
	shovelerPositionQuantizerFromWorld(
		&quantizer, worldMax, &quantizedX, &quantizedY);
	ASSERT_EQ(quantizedX, UINT64_MAX);
	ASSERT_EQ(quantizedY, UINT64_MAX);

	auto quantize_dequantize = [&](ShovelerVector2 input) {
		shovelerPositionQuantizerFromWorld(
			&quantizer, input, &quantizedX, &quantizedY);
