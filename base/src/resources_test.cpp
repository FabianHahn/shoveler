...
public:
  ...
  virtual void TearDown() {
    ...
    ASSERT_EQ(*freeResourceDataArguments.rbegin(), testTypeLoader.defaultResourceData)
        << "type loader's default resource data should be freed after resources was freed";
...
TEST_F(ShovelerResourcesTest, requestUnloaded) {
  ...
  ASSERT_EQ(lastRequestResourceId, testResourceId)
      << "request should be called with correct resource id";
  ...
  ASSERT_EQ(resource->data, testDefaultResourceData)
      << "returned resource should have correct default data";
}
...
TEST_F(ShovelerResourcesTest, loadUnrequested) {
  ...
  bool loaded = shovelerResourcesSet(
      resources, testTypeId, testResourceId, &testResourceBuffer, testResourceBufferSize);
  ...
  ASSERT_EQ(lastLoadBufferSize, testResourceBufferSize)
      << "load should be called with correct bytes";
}
...
TEST_F(ShovelerResourcesTest, loadInvalidType) {
  ...
  bool loaded = shovelerResourcesSet(
      resources, testInvalidTypeId, testResourceId, &testResourceBuffer, testResourceBufferSize);
...
TEST_F(ShovelerResourcesTest, freeLoaded) {
  ...
  ASSERT_EQ(*freeResourceDataArguments.begin(), testResourceData)
      << "free resource data should be called with correct resource data";
...
TEST_F(ShovelerResourcesTest, requestAndFailToLoad) {
  ...
  ASSERT_EQ(resource->data, testDefaultResourceData)
      << "resource data should be unchanged after failing to load";
}
...
TEST_F(ShovelerResourcesTest, loadAndRequest) {
  ...
  ASSERT_EQ(resource->data, testResourceData)
      << "resource data should be set to correct loaded data";
}
