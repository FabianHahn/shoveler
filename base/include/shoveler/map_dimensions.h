...
static inline bool shovelerMapIsTileInBounds(
    const ShovelerMapDimensions* mapDimensions, ShovelerMapTileCoordinate tile) {
  return tile.chunkX >= 0 && tile.chunkX < mapDimensions->numChunkColumns && tile.chunkY >= 0 &&
      tile.chunkY < mapDimensions->numChunkRows && tile.tileX >= 0 &&
      tile.tileX < mapDimensions->chunkSize && tile.tileY >= 0 &&
...
static inline ShovelerPositionQuantizer shovelerPositionQuantizerFromMapDimensions(
  ...
  return shovelerPositionQuantizer(
      ...
      /* worldSize */
      shovelerVector2(
          2.0f * (float) mapDimensions->halfMapWidth, 2.0f * (float) mapDimensions->halfMapHeight));
}
