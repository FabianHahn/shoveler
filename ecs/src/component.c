...
static bool activateSystem(ShovelerComponent *component);
static void deactivateSystem(ShovelerComponent *component);
static void updateReverseDependency(ShovelerComponent *sourceComponent, ShovelerComponent *targetComponent, void *unused);
static void activateReverseDependencySystem(ShovelerComponent *sourceComponent, ShovelerComponent *targetComponent, void *unused);
static void deactivateReverseDependencySystem(ShovelerComponent *sourceComponent, ShovelerComponent *targetComponent, void *unused);
static void addFieldDependencies(ShovelerComponent *component, const ShovelerComponentField *field, const ShovelerComponentFieldValue *fieldValue);
static void removeFieldDependencies(ShovelerComponent *component, const ShovelerComponentField *field, const ShovelerComponentFieldValue *fieldValue);
...
static bool checkDependenciesActiveSystem(ShovelerComponent *component);
...
ShovelerComponent *shovelerComponentCreate(ShovelerComponentWorldAdapter *worldAdapter, ShovelerComponentSystemAdapter *systemAdapter, long long int entityId, ShovelerComponentType *componentType)
{
	...
	if(component->type->numFields > 0) {
		component->fieldValues = malloc((size_t) component->type->numFields * sizeof(ShovelerComponentFieldValue));
...
{
	assert(id >= 0);
	assert(id < component->type->numFields);
	...
	if(!isCanonical) {
		...
		component->worldAdapter->updateAuthoritativeComponent(component, field, value, component->worldAdapter->userData);
	}

	bool wasActive = component->systemData != NULL;
	bool canLiveUpdate = component->systemAdapter->canLiveUpdateField(component, field, component->systemAdapter->userData);
	...
	if(wasActive) {
		if(canLiveUpdate) {
			// live update value
			bool propagateUpdate = component->systemAdapter->liveUpdateField(component, field, fieldValue, component->systemAdapter->userData);

			if(propagateUpdate) {
				// update reverse dependencies
				component->worldAdapter->forEachReverseDependency(component, updateReverseDependency, /* callbackUserData */ NULL, component->worldAdapter->userData);
			}
		} else {
			// cannot live update, so try reactivating again
			activateSystem(component);
		}
	}
...
bool shovelerComponentClearfield(ShovelerComponent *component, int id, bool isCanonical)
{
	...
	return shovelerComponentUpdateField(component, id, &fieldValue, isCanonical);
}
...
const ShovelerComponentFieldValue *shovelerComponentGetFieldValue(ShovelerComponent *component, int id)
{
	...
	return &component->fieldValues[id];
}
...
{
	if(!shovelerComponentIsSystemActive(component)) {
		return false;
	}

	if(!component->systemAdapter->updateSystem(component, dt, component->systemAdapter->userData)) {
		return false;
	}
...
{
	bool requiresAuthority = component->systemAdapter->requiresAuthority(component, component->systemAdapter->userData);
	if(shovelerComponentIsSystemActive(component) && requiresAuthority) {
		deactivateSystem(component);
	}
...
ShovelerComponent *shovelerComponentGetDependency(ShovelerComponent *component, int fieldId)
{
	...
	return component->worldAdapter->getComponent(component, toDependencyTargetEntityId(component, fieldValue->entityIdValue), field->dependencyComponentTypeId, component->worldAdapter->userData);
}
...
ShovelerComponent *shovelerComponentGetArrayDependency(ShovelerComponent *component, int fieldId, int index)
{
	...
	return component->worldAdapter->getComponent(component, toDependencyTargetEntityId(component, fieldValue->entityIdArrayValue.entityIds[index]), field->dependencyComponentTypeId, component->worldAdapter->userData);
}
...
static bool activateSystem(ShovelerComponent *component)
{
	...
	if(!checkDependenciesActiveSystem(component)) {
		return false;
	}

	component->systemData = component->systemAdapter->activateSystem(component, component->systemAdapter->userData);
	...
	component->worldAdapter->forEachReverseDependency(component, activateReverseDependencySystem, /* callbackUserData */ NULL, component->worldAdapter->userData);
...
static void deactivateSystem(ShovelerComponent *component)
{
	...
	component->worldAdapter->forEachReverseDependency(component, deactivateReverseDependencySystem, /* callbackUserData */ NULL, component->worldAdapter->userData);

	component->systemAdapter->deactivateSystem(component, component->systemAdapter->userData);
	component->systemData = NULL;
...
static void updateReverseDependency(ShovelerComponent *sourceComponent, ShovelerComponent *targetComponent, void *unused)
{
	...
	for(int fieldId = 0; fieldId < sourceComponent->type->numFields; fieldId++) {
		...
		if(field->type == SHOVELER_COMPONENT_FIELD_TYPE_ENTITY_ID) {
			if(toDependencyTargetEntityId(sourceComponent, fieldValue->entityIdValue) != targetComponent->entityId) {
				continue;
			}
		} else {
			...
			for(int i = 0; i < fieldValue->entityIdArrayValue.size; i++) {
				if(toDependencyTargetEntityId(sourceComponent, fieldValue->entityIdArrayValue.entityIds[i]) == targetComponent->entityId) {
					requiresUpdate = true;
					break;
				}
			}
		...
		if(sourceComponent->systemAdapter->canLiveUpdateDependencyField(
			   sourceComponent, field, sourceComponent->systemAdapter->userData)) {
		...
		bool propagateUpdate = sourceComponent->systemAdapter->liveUpdateDependencyField(sourceComponent, field, targetComponent, sourceComponent->systemAdapter->userData);
		if(propagateUpdate) {
			// recursively update reverse dependencies
			sourceComponent->worldAdapter->forEachReverseDependency(sourceComponent, updateReverseDependency, /* callbackUserData */ NULL, sourceComponent->worldAdapter->userData);
		}
	}

	if(requiresReactivation) {
		// reactivate the source component system
		deactivateSystem(sourceComponent);
		activateSystem(sourceComponent);
	}
}
...
static void addFieldDependencies(ShovelerComponent *component, const ShovelerComponentField *field, const ShovelerComponentFieldValue *fieldValue)
{
	...
	if(field->type == SHOVELER_COMPONENT_FIELD_TYPE_ENTITY_ID) {
		...
		for(int i = 0; i < fieldValue->entityIdArrayValue.size; i++) {
			addDependency(
				component,
				/* targetEntityId */ toDependencyTargetEntityId(component, fieldValue->entityIdArrayValue.entityIds[i]),
				/* targetComponentTypeId */ field->dependencyComponentTypeId);
		}
	}
...
static void addDependency(ShovelerComponent *component, long long int targetEntityId, const char *targetComponentTypeId)
{
	...
	component->worldAdapter->addDependency(component, targetEntityId, targetComponentTypeId, component->worldAdapter->userData);
}
...
static void removeDependency(ShovelerComponent *component, long long int targetEntityId, const char *targetComponentTypeId)
{
	...
	bool dependencyRemoved = component->worldAdapter->removeDependency(component, targetEntityId, targetComponentTypeId, component->worldAdapter->userData);
...
static bool checkDependenciesActiveSystem(ShovelerComponent *component)
{
	for(GList *iter = component->dependencies->head; iter != NULL; iter = iter->next) {
		Dependency *dependency = iter->data;
		ShovelerComponent *targetComponent = component->worldAdapter->getComponent(component, dependency->entityId, dependency->componentTypeId, component->worldAdapter->userData);
		if(targetComponent == NULL) {
			return false;
		}

		if(!shovelerComponentIsSystemActive(targetComponent)) {
			return false;
		}
	}
