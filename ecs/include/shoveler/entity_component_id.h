...
static inline ShovelerEntityComponentId* shovelerEntityComponentIdCreate(
    long long int entityId, const char* componentTypeId) {
  ShovelerEntityComponentId *entityComponentId = malloc(sizeof(ShovelerEntityComponentId));
  entityComponentId->entityId = entityId;
  entityComponentId->componentTypeId = componentTypeId;
  return entityComponentId;
}
...
static inline gboolean shovelerEntityComponentIdEqual(gconstpointer aPointer, gconstpointer bPointer) {
  const ShovelerEntityComponentId* a = aPointer;
  const ShovelerEntityComponentId* b = bPointer;
