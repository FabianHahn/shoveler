...
static void shovelerSampleInit(ShovelerGame *sampleGame, int width, int height, int samples)
{
	...
	tiles = shovelerDrawableTilesCreate(2, 2);
	tilemapMaterial = shovelerMaterialTilemapCreate();
	shovelerMaterialTilemapSetSource(tilemapMaterial, tilemapTexture);
	shovelerMaterialTilemapSetTileset(tilemapMaterial, 0, texture, interpolatingSampler, 2, 2);
