...
int main(int argc, char *argv[])
{
	...
	tileSprite->translation = shovelerVector2(-0.3f, -0.2f);
	tileSprite->scale = shovelerVector2(0.25f, 0.4f);
	shovelerCanvasAddSprite(canvas, tileSprite);

	ShovelerCollider2 tileBoxCollider = shovelerColliderBox2(shovelerBoundingBox2(
		shovelerVector2LinearCombination(1.0f, tileSprite->translation, -0.5f, tileSprite->scale),
		shovelerVector2LinearCombination(1.0f, tileSprite->translation, 0.5f, tileSprite->scale)));
	...
	characterSprite->translation = shovelerVector2(0.0f, 0.0f);
	characterSprite->scale = shovelerVector2(0.2f, 0.2f);
...
static void shovelerSampleUpdate(ShovelerGame *game, double dt)
{
	...
	float moveAmountX = controller->frame.position.values[0] - characterSprite->translation.values[0];
	float moveAmountY = controller->frame.position.values[1] - characterSprite->translation.values[1];
	shovelerTileSpriteAnimationUpdate(animation, shovelerVector2(moveAmountX, moveAmountY));

	characterSprite->translation.values[0] = controller->frame.position.values[0];
	characterSprite->translation.values[1] = controller->frame.position.values[1];
}
