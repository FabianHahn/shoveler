...
int main(int argc, char* argv[]) {
  ...
  colorMaterial = shovelerMaterialColorCreate(
      game->shaderCache, /* screenspace */ false, shovelerVector4(0.7, 0.7, 0.7, 1.0));
  ...
  textureMaterial = shovelerMaterialTextureCreate(
      game->shaderCache,
      /* screenspace */ false,
  ...
  ShovelerLight* pointlight = shovelerLightPointCreate(
      game->shaderCache,
  ...
  ShovelerLight* pointlight2 = shovelerLightPointCreate(
      game->shaderCache,
      shovelerVector3(-8, -8, -8),
      1024,
      1024,
      1,
      0.0f,
      80.0f,
      shovelerVector3(0.1f, 0.1f, 0.1f));
  shovelerSceneAddLight(game->scene, pointlight2);

  point = shovelerDrawablePointCreate();
  particleMaterial =
      shovelerMaterialParticleCreate(game->shaderCache, shovelerVector4(1.0f, 1.0f, 1.0f, 1.0f));
  ...
  screenspaceTextureMaterial = shovelerMaterialTextureCreate(
      game->shaderCache,
      /* screenspace */ true,
      SHOVELER_MATERIAL_TEXTURE_TYPE_DEPTH,
      shovelerLightPointGetShared(pointlight)->depthFramebuffer->depthTarget,
      /* manageTexture */ false,
      nearestNeighborSampler,
      /* manageSampler */ false);
