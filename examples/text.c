...
int main(int argc, char *argv[])
{
	...
	ShovelerGame *game = shovelerGameCreate(shovelerSampleUpdate, /* updateAuthoritativeViewComponent */ NULL, /* updateAuthoritativeViewComponentUserData */ NULL, &windowSettings, &cameraSettings, &controllerSettings);
	if(game == NULL) {
		return EXIT_FAILURE;
	}
...
static void shovelerSampleUpdate(ShovelerGame *game, double dt)
{
	...
	if(frameTimeSinceLastUpdate > 0.1) {
		...
		screenspaceTextSprite->position = shovelerVector2(10.0f, game->framebuffer->height - 48.0f - 10.0f);
