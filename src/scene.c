...
void shovelerSceneAddLight(ShovelerScene *scene, ShovelerLight *light)
{
	...
	scene->depthTextureGaussianFilterXMaterial = shovelerMaterialDepthTextureGaussianFilterGaussianFilterCreate(light->framebuffer->depthTarget, false, light->framebuffer->width, light->framebuffer->height);
	shovelerMaterialDepthTextureGaussianFilterEnableExponentialLifting(scene->depthTextureGaussianFilterXMaterial, 80.0f);
	scene->depthTextureGaussianFilterYMaterial = shovelerMaterialDepthTextureGaussianFilterGaussianFilterCreate(scene->depthTextureGaussianFilterXFramebuffer->renderTarget, false, light->framebuffer->width, light->framebuffer->height);
	shovelerMaterialDepthTextureGaussianFilterSetDirection(scene->depthTextureGaussianFilterYMaterial, false, true);
