...
static_assert(sizeof(GHashTableIterPrivate) <= sizeof(GHashTableIter),
		"the size of GHashTableIterPrivate exceeds the size of GHashTableIter on your platform");
...
static void removeMapElement(GHashTable* hashTable, GHashTable::Map::iterator iter)
{
	assert(&hashTable->functions == iter->first.functions);
	assert(&hashTable->functions == iter->second.functions);

	// Copy the key and mapped value so we can free them after the element has been removed.
	void* storedKeyValue = iter->first.value;
	void* storedMappedValue = iter->second.value;

	// Remove the entry from the table. This might actually still access the entry so we can't
	// have freed the values yet at this point.
	hashTable->map.erase(iter);

	if (hashTable->functions.keyDestroy != NULL) {
		hashTable->functions.keyDestroy(storedKeyValue);
	}
	if (hashTable->functions.mappedDestroy != NULL) {
		hashTable->functions.mappedDestroy(storedMappedValue);
	}
}
...
FAKEGLIB_API GHashTable*
g_hash_table_new_full(GHashFunc hash_func, GEqualFunc key_equal_func, GDestroyNotify key_destroy_func,
...
FAKEGLIB_API gboolean g_hash_table_insert(GHashTable* hashTable, gpointer keyValue, gpointer mappedValue)
{
	...
	// Destroy the passed in key since it is now owned by the table but we don't need it since
	// there is one already.
	if (hashTable->functions.keyDestroy != NULL) {
		hashTable->functions.keyDestroy(keyValue);
	}
	// Destroy the previous mapped value since we're overwriting it with a new one.
	if (hashTable->functions.mappedDestroy != NULL) {
...
FAKEGLIB_API gboolean g_hash_table_replace(GHashTable* hashTable, gpointer keyValue, gpointer mappedValue)
{
	...
	if (query != hashTable->map.end()) {
		removeMapElement(hashTable, query);
		newlyInserted = false;
	}
...
FAKEGLIB_API gboolean
g_hash_table_lookup_extended(GHashTable* hashTable, gconstpointer lookupKeyValue, gpointer* origKeyValue,
		gpointer* mappedValue)
{
...
FAKEGLIB_API guint g_hash_table_foreach_remove(GHashTable* hashTable, GHRFunc func, gpointer userData)
{
	...
	for (GHashTable::Map::iterator iter = hashTable->map.begin(); iter != hashTable->map.end();) {
		if (func(iter->first.value, iter->second.value, userData)) {
			removeMapElement(hashTable, iter++);
			removed++;
		} else {
			++iter;
		}
...
FAKEGLIB_API void g_hash_table_iter_remove(GHashTableIter* iter)
{
	...
	removeMapElement(privateIter->hashTable, privateIter->iter++);
